...
def mapPlotly(merge_df, month):
    ...
    # Sidebar input for map style selection
    mapbox_style = st.sidebar.selectbox('Map Style', 
    ...
    fig = px.choropleth_mapbox(result,
                               ...
                               opacity=0.5,
                               labels={'mean': 'Average Apartment Price (10K KRW)'})

    fig.update_layout(margin=dict(r=0, t=0, l=0, b=0))
    fig.update_traces(hovertemplate='<b>%{location}</b><br>Avg Price: %{z:,.0f} (10K KRW)')
